...
**① Visual break — RULED (owner, 2026-07-13): Hybrid — BP-4 structure, lotus flush-face skin.** Adopt BP-4's IA *exactly* (rail-of-10 + Notes-seam, two-tab Spaces|Vault panel, global tabs + **melt**, five-lens right panel, Layers) but render it in the shipped flush-face aesthetic — the **middle is the bare window material**, the rail + left panel + right panel are **opaque floating cards on that face** (commit 2aa0cc4's model), with **minimal borders**, not BP-4's bordered-regions + `chrome→canvas→surface` shade ladder. So: keep the *structure*, drop the *shade ladder*. The melt still ships — but the active tab melts into the **material face**, not a bordered canvas band (which makes it *easier*: no seam-cover `::after` strip is needed when there's no divider to hide — the face is continuous by construction; §17d's SwiftUI recipe simplifies to "active tab paints nothing, just weight/lift"). Every "canvas band" / "surface fill" the ledger and slices reference collapses to the material face; every "panel" is a `.panelCard()` float. Palette laws unchanged (lake-green selection-only, amber AI-only, value-hue dots).
